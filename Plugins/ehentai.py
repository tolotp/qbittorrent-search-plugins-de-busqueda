...
class ehentai(object):
    ...
    def search(self, what, cat='all'):
        ...
        while page < MAX_PAGES:
            search_url = f"{self.url}/torrents.php?search={query}&page={page}"
            ...
            all_matches = pattern.findall(html_content)
            
            if not all_matches and page == 0:
                prettyPrinter({
                    'link': self.url,
            ...
            for match in all_matches:
                unique_id = f"{match[2]}_{match[3]}_{match[1]}"
                if unique_id not in seen_torrents:
